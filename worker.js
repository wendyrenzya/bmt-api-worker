...
// ======================================================
// RIWAYAT ALL — FULL DATA (OPT-IN, AMAN)
// ======================================================
async function riwayatAllFull(env, url) {
  const limit  = Number(url.searchParams.get("limit")  || 10);
  const offset = Number(url.searchParams.get("offset") || 0);
  ...
  const q = await env.BMT_DB.prepare(`
    ...
    SELECT
      ...
      -- tipe prioritas
      CASE
        WHEN SUM(tipe='servis')  > 0 THEN 'servis'
        WHEN SUM(tipe='charge')  > 0 THEN 'servis'
        WHEN SUM(tipe='keluar')  > 0 THEN 'penjualan'
        WHEN SUM(tipe='masuk')   > 0 THEN 'masuk'
        WHEN SUM(tipe='audit')   > 0 THEN 'audit'
        ELSE 'lain'
      END AS tipe,
      ...
      -- preview barang
      MAX(CASE WHEN rn=1 AND tipe='keluar' THEN barang_nama END) AS first_barang_nama,
      MAX(CASE WHEN rn=1 AND tipe='keluar' THEN jumlah END)      AS first_barang_qty,
  ...
  const items = (q.results || []).map(r => {
    ...
    return {
      ...
      barang: {
        ada: barang > 0,
        total: barang,
        first_nama: r.first_barang_nama || null,
        first_qty: r.first_barang_qty || 0,
