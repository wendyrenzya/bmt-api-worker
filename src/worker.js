// worker.js — BMT API (final)
// Binding required: D1 database with name "DB"
// Note: keep this file as single top-level JS file for easy paste
...
export default {
  async fetch(request, env) {
    ...
    // Basic CORS (handle preflight and add CORS headers to JSON responses)
    const CORS_HEADERS = {
    ...
    const withCors = (resp) => {
      for (const k of Object.keys(CORS_HEADERS)) {
        resp.headers.set(k, CORS_HEADERS[k]);
      }
      return resp;
    };
    ...
    const checkRole = async (username, allowed = []) => {
      ...
      if (!allowed.includes(user.role)) return { ok: false, code: 403, msg: 'Anda tidak memiliki akses, hubungi owner' , user};
      return { ok: true, user };
    };

    // ---------- Parse path ----------
    const parts = path.split('/').filter(Boolean); // skip empty

    try {
      // Health
      if ((path === '' || path === '/') && method === 'GET') {
        return withCors(text('BMT API OK'));
      }
      ...
      if (path === '/auth/login' && method === 'POST') {
        ...
        if (!username || !password) {
          return withCors(json({ ok: false, error: 'username & password required' }, 400));
        }
        const row = await dbFirst('SELECT username,name,role,photo_url,password FROM users WHERE username = ?', username);
        if (!row) return withCors(json({ ok: false, error: 'invalid credentials' }, 401));
        if (row.password !== password) return withCors(json({ ok: false, error: 'invalid credentials' }, 401));
      ...
      if (path === '/auth/me' && method === 'GET') {
        ...
        if (!u) return withCors(json({ ok: false, error: 'X-User header required' }, 401));
        const rec = await getUserRecord(u);
        if (!rec) return withCors(json({ ok: false, error: 'user not found' }, 404));
        return withCors(json({ ok: true, user: rec }));
      }

      // ---------- USERS ----------
      if (path === '/users' && method === 'GET') {
        const perm = await checkRole(getHeaderUser(request), ['owner', 'admin']);
        if (!perm.ok) return withCors(json({ ok: false, error: perm.msg }, perm.code));
        const res = await dbAll('SELECT username,name,role,photo_url,created_at FROM users ORDER BY created_at DESC');
        return withCors(json({ ok: true, results: res.results || [] }));
      }

      if (path === '/users' && method === 'POST') {
        // create user — owner only; if users table empty allow bootstrap creation as owner
        const body = await safeJson(request);
        if (!body || !body.username || !body.password || !body.name) return withCors(json({ ok: false, error: 'username,password,name required' }, 400));

        // check if any user exists
        const cntRes = await dbFirst("SELECT COUNT(*) AS c FROM users");
        const c = (cntRes && cntRes.c) ? cntRes.c : 0;
        if (c === 0) {
          // bootstrap: create owner
          await dbRun('INSERT INTO users (username,password,name,role,photo_url,created_at) VALUES (?,?,?,?,?,CURRENT_TIMESTAMP)',
            body.username, body.password, body.name, 'owner', body.photo_url || '');
          return withCors(json({ ok: true, message: 'owner created (bootstrap)' }, 201));
        }

        const caller = getHeaderUser(request);
        if (!caller) return withCors(json({ ok: false, error: 'X-User header required' }, 401));
        ...
        try {
          ...
          return withCors(json({ ok: true, message: 'user created' }, 201));
        } catch (err) {
          return withCors(json({ ok: false, error: String(err) }, 400));
        }
      }
      ...
      if (parts[0] === 'users' && parts[1] && (method === 'PUT' || method === 'PATCH')) {
        ...
        if (!callerRec) return withCors(json({ ok: false, error: 'caller not found' }, 401));
        if (callerRec.role !== 'owner' && caller !== target) return withCors(json({ ok: false, error: 'forbidden' }, 403));

        const body = await safeJson(request);
        if (!body) return withCors(json({ ok: false, error: 'body required' }, 400));
        ...
        if (fields.length === 0) return withCors(json({ ok: false, error: 'nothing to update' }, 400));

        binds.push(target);
        const sql = `UPDATE users SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE username = ?`;
        try {
          await dbRun(sql, ...binds);
          return withCors(json({ ok: true, message: 'updated' }));
        } catch (err) {
      ...
      if (parts[0] === 'users' && parts[1] && method === 'DELETE') {
        ...
        try {
          ...
          return withCors(json({ ok: true, message: 'deleted' }));
        } catch (err) {
          return withCors(json({ ok: false, error: String(err) }, 500));
        }
      }
      ...
      if (parts[0] === 'barang' && parts[1] && method === 'GET') {
        ...
        if (!id) return withCors(json({ ok: false, error: 'invalid id' }, 400));
        const r = await dbFirst('SELECT * FROM barang WHERE id = ?', id);
        if (!r) return withCors(json({ ok: false, error: 'not found' }, 404));
        return withCors(json({ ok: true, result: r }));
      }
      ...
      if (path === '/barang' && method === 'POST') {
        ...
        if (!perm.ok) return withCors(json({ ok: false, error: perm.msg }, perm.code));
        const body = await safeJson(request);
        if (!body || !body.nama) return withCors(json({ ok: false, error: 'nama required' }, 400));
        try {
          const res = await dbRun(
            `INSERT INTO barang (kode_barang,nama,harga,harga_modal,stock,kategori,foto,deskripsi,created_at) VALUES (?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)`,
          ...
          // riwayat add
          await dbRun('INSERT INTO riwayat (tipe, barang_id, barang_nama, jumlah, catatan, dibuat_oleh, created_at) VALUES (?,?,?,?,?,?,CURRENT_TIMESTAMP)',
            'tambah_barang', newId, body.nama, Number(body.stock || 0), body.catatan || '', perm.user.username);
          return withCors(json({ ok: true, id: newId }));
        } catch (err) {
          return withCors(json({ ok: false, error: String(err) }, 500));
        }
      }

      // Update barang PUT /barang/:id
      if (parts[0] === 'barang' && parts[1] && (method === 'PUT' || method === 'PATCH')) {
        ...
        if (!cur) return withCors(json({ ok: false, error: 'not found' }, 404));
      ...
      if (path === '/stok_masuk' && method === 'POST') {
        ...
        if (!qty || qty <= 0) return withCors(json({ ok: false, error: 'jumlah must be > 0' }, 400));

        // resolve barang by id, kode_barang, or nama
        ...
        // if no id but nama present, create minimal barang
        let barangRow = null;
        if (!bid && body.nama) {
          const res = await dbRun('INSERT INTO barang (kode_barang,nama,harga,harga_modal,stock,created_at) VALUES (?,?,?,?,?,CURRENT_TIMESTAMP)',
          ...
          return withCors(json({ ok: false, error: 'barang_id or kode_barang or nama required' }, 400));
        }

        if (!barangRow) return withCors(json({ ok: false, error: 'barang not found' }, 404));
        ...
        try {
          ...
          return withCors(json({ ok: true, message: 'stok updated', barang_id: bid, newStock }));
        } catch (err) {
          return withCors(json({ ok: false, error: String(err) }, 500));
        }
      }
      ...
      if (path === '/stok_keluar' && method === 'POST') {
        ...
        if (!bid) return withCors(json({ ok: false, error: 'barang_id or kode_barang or nama required' }, 400));

        const barangRow = await dbFirst('SELECT id,nama,stock FROM barang WHERE id = ?', bid);
        if (!barangRow) return withCors(json({ ok: false, error: 'barang not found' }, 404));
        if (Number(barangRow.stock || 0) < qty) return withCors(json({ ok: false, error: 'stock tidak mencukupi' }, 400));
      ...
      if (path === '/riwayat' && method === 'GET') {
        ...
        const from = params.from || null; // ISO date or date-string
      ...
      if (path === '/more' && (method === 'PUT' || method === 'POST')) {
        ...
        try {
          ...
          return withCors(json({ ok: true, message: 'more updated' }));
        } catch (err) {
          return withCors(json({ ok: false, error: String(err) }, 500));
        }
      }
      ...
      return withCors(json({ ok: false, error: msg }, 500));
    }
  }
};
